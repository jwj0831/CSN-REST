CREATE TABLE IF NOT EXISTS `csn_db`.`csn_options` (
...
CREATE TABLE IF NOT EXISTS `csn_db`.`csn_snsr_meta` (
...
CREATE TABLE IF NOT EXISTS `csn_db`.`csn_snsr_options` (
  `snsr_id` VARCHAR(45) NOT NULL,
  ...
  CONSTRAINT `snsr_id_in_opt`
    FOREIGN KEY (`snsr_id`)
    REFERENCES `csn_db`.`csn_snsr_meta` (`snsr_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;


CREATE TABLE IF NOT EXISTS `csn_db`.`csn_snsr_tags` (
  `snsr_id` VARCHAR(45) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `csn_db`.`csn_sn_meta` (
...
CREATE TABLE IF NOT EXISTS `csn_db`.`csn_sn_options` (
  `sn_id` VARCHAR(45) NOT NULL,
  `opt_name` VARCHAR(255) NOT NULL,
  `opt_val` VARCHAR(255) NOT NULL,
  ...
  CONSTRAINT `sn_id_in_opt`
    FOREIGN KEY (`sn_id`)
    REFERENCES `csn_db`.`csn_sn_meta` (`sn_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;


CREATE TABLE IF NOT EXISTS `csn_db`.`csn_sn_tags` (
  `sn_id` VARCHAR(45) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `csn_db`.`csn_sn_members` (
  `sn_id` VARCHAR(45) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `csn_db`.`csn_snsr_persistence` (
...
CREATE TABLE IF NOT EXISTS `csn_db`.`csn_user` (
...
CREATE TABLE IF NOT EXISTS `csn_db`.`csn_auth_key` (
  ...
  CONSTRAINT `user_id_auth`
    FOREIGN KEY (`user_id`)
    REFERENCES `csn_db`.`csn_user` (`user_id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB;